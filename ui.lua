...
return function(Engine, catalog, host, inheritedParent, Icons)
    ...
    local function icon(parent, name, size, color, pos, anchor, glyph)
        ...
        local l = label(parent, { Position = pos, AnchorPoint = anchor, Size = UDim2.fromOffset(size, size) }, glyph or "â€¢", size, false, color, Enum.TextXAlignment.Center)
        return l
    end
    ...
    local DEFAULTS = {
        ...
        disableaccidents = false,
        disablefeaturedsongs = false,
        alwaysshowmidispoofer = false,
        errorMargin = 0,
        midiSpoof = false,
    }
    ...
    local function loadSettings()
        ...
        if hasRead() then
            local ok, raw = pcall(function()
                return isfile(SETTINGS_PATH) and readfile(SETTINGS_PATH) or nil
            end)
    ...
    local function saveSettings()
        ...
        pcall(function()
            if hasMkdir() and not (type(isfolder) == "function" and isfolder(DIR)) then makefolder(DIR) end
    ...
    corner(toggle, 22)
    icon(toggle, "music", 22, Color3.fromRGB(0, 0, 0), UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5), "â™«")
    ...
    icon(brand, "music", 22, C.text, UDim2.new(0, 18, 0.5, 0), Vector2.new(0, 0.5), "â™«")
    label(brand, { Position = UDim2.new(0, 48, 0, 0), Size = UDim2.new(1, -56, 1, 0) }, "0M3G4 P1AN0", 16, true, C.text)

    local nav = make("Frame", { Parent = sidebar, Position = UDim2.new(0, 0, 0, 56), Size = UDim2.new(1, 0, 0, 44), BackgroundTransparency = 1 })
    ...
    icon(browseBtn, "home", 18, C.text, UDim2.new(0, 8, 0.5, 0), Vector2.new(0, 0.5), "âŒ‚")
    ...
    icon(settingsBtn, "sliders", 18, C.sub, UDim2.new(0, 8, 0.5, 0), Vector2.new(0, 0.5), "âš™")
    ...
    corner(searchBox, 17)
    icon(searchBox, "search", 16, C.sub, UDim2.new(0, 12, 0.5, 0), Vector2.new(0, 0.5), "âŒ•")
    ...
    local gearBtn = make("TextButton", { Parent = top, Position = UDim2.new(1, -116, 0, 16), Size = UDim2.fromOffset(34, 34), BackgroundColor3 = C.elev, BorderSizePixel = 0, Text = "", AutoButtonColor = true })
    corner(gearBtn, 17)
    icon(gearBtn, "settings", 16, C.text, UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5), "âš™")
    local minBtn = make("TextButton", { Parent = top, Position = UDim2.new(1, -76, 0, 16), Size = UDim2.fromOffset(34, 34), BackgroundColor3 = C.elev, BorderSizePixel = 0, Text = "", AutoButtonColor = true })
    corner(minBtn, 17)
    icon(minBtn, "minus", 16, C.text, UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5), "â€“")
    local closeBtn = make("TextButton", { Parent = top, Position = UDim2.new(1, -36, 0, 16), Size = UDim2.fromOffset(34, 34), BackgroundColor3 = C.elev, BorderSizePixel = 0, Text = "", AutoButtonColor = true })
    corner(closeBtn, 17)
    icon(closeBtn, "x", 16, C.text, UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5), "Ã—")
    ...
    -- shared hover play button
    local hoverPlay = make("ImageLabel", { Parent = list, Size = UDim2.fromOffset(16, 16), BackgroundTransparency = 1, Visible = false, ZIndex = 5 })
    local hoverSong = nil
    if IIcons then setIcon(hoverPlay, "play", C.accent) end
    local hoverHit = make("TextButton", { Parent = list, Size = UDim2.fromOffset(22, 22), BackgroundTransparency = 1, Text = "", Visible = false, ZIndex = 6 })
    hoverHit.MouseButton1Click:Connect(function() if hoverSong then selectSong(hoverSong) end end)

    -- ------------------------------------------------------------------ now bar
    local now = make("Frame", { Parent = win, Position = UDim2.new(0, 0, 1, -NOW_H), Size = UDim2.new(1, 0, 0, NOW_H), BackgroundColor3 = C.panel, BorderSizePixel = 0 })
    ...
    corner(art, 8)
    make("UIGradient", { Parent = art, Color = ColorSequence.new(Color3.fromRGB(30, 215, 96), Color3.fromRGB(20, 120, 80)), Rotation = 45 })
    icon(art, "music", 24, Color3.fromRGB(255, 255, 255), UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5), "â™«")

    local npTitle = label(now, { Position = UDim2.new(0, 88, 0, 20), Size = UDim2.new(0, 230, 0, 18) }, "Nothing playing", 14, true, C.text)
    npTitle.TextTruncate = Enum.TextTruncate.AtEnd
    local npSub = label(now, { Position = UDim2.new(0, 88, 0, 40), Size = UDim2.new(0, 230, 0, 16) }, "pick a song", 11, false, C.sub)
    ...
    local function transportBtn(name, glyph, size, isPrimary)
        local b = make("TextButton", { Parent = btns, Size = UDim2.fromOffset(size, size), BackgroundColor3 = isPrimary and C.accent or Color3.fromRGB(0, 0, 0), BackgroundTransparency = isPrimary and 0 or 1, BorderSizePixel = 0, Text = "", AutoButtonColor = false })
        if isPrimary then corner(b, size / 2) end
        icon(b, name, size * 0.5, isPrimary and Color3.fromRGB(0, 0, 0) or C.sub, UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5), glyph)
        return b
    end
    local shuffleBtn = transportBtn("shuffle", "â‡„", 26, false)
    local playBtn = transportBtn("play", "â–¶", 38, true)
    local stopBtn = transportBtn("square", "â– ", 26, false)
    ...
    local function pill(order, iconName, glyph, width)
        ...
        icon(minus, "minus", 12, C.text, UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5), "âˆ’")
    ...
    local _, bpmMinus, bpmVal, bpmPlus = pill(1, "clock", "", 96)
    local _, errMinus, errVal, errPlus = pill(2, "zap", "", 92)
    ...
    local panelHead = make("Frame", { Parent = panel, Size = UDim2.new(1, 0, 0, 56), BackgroundTransparency = 1 })
    label(panelHead, { Position = UDim2.new(0, 20, 0, 0), Size = UDim2.new(1, -60, 1, 0) }, "Settings", 18, true, C.text)
    local panelClose = make("TextButton", { Parent = panelHead, Position = UDim2.new(1, -46, 0, 12), Size = UDim2.fromOffset(32, 32), BackgroundColor3 = C.elev, BorderSizePixel = 0, Text = "", AutoButtonColor = true })
    corner(panelClose, 16)
    icon(panelClose, "x", 15, C.text, UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5), "Ã—")

    local settingsList = make("ScrollingFrame", { Parent = panel, Position = UDim2.new(0, 12, 0, 60), Size = UDim2.new(1, -24, 1, -72), BackgroundTransparency = 1, BorderSizePixel = 0, ScrollBarThickness = 4, CanvasSize = UDim2.new(0, 0, 0, 0), AutomaticCanvasSize = Enum.AutomaticSize.Y })
    make("UIListLayout", { Parent = settingsList, Padding = UDim.new(0, 6), SortOrder = Enum.SortOrder.LayoutOrder })
    ...
    local function getSongChunk(song)
        ...
        -- 2. executor cache (only when "cache songs" is on)
    ...
    local function setTransportIcon()
        local playing = Engine.playing and not Engine.paused
        setIcon(playBtn, playing and "pause" or "play", playing and Color3.fromRGB(0, 0, 0) or Color3.fromRGB(0, 0, 0))
    end

    local function updateProgress(beat)
        local total = Engine.totalBeats
        local p = (total and total > 0) and ((beat or Engine.position) / total) or 0
    ...
    local function updateNow(song)
        ...
        local tags = (#song.cat > 0) and table.concat(song.cat, " Â· ") or "untagged"
        npSub.Text = string.format("%s BPM  â€¢  %s", tostring(song.bpm), tags)
    end
    ...
    function selectSong(song)
        ...
        if Settings.data.autoplay then
            ...
            -- loaded and paused; press play to start
            setTransportIcon()
    ...
    local function matches(song)
        ...
        if query ~= "" then
            local q = query:lower()
            if not (song.name:lower():find(q, 1, true) or (song.file or ""):lower():find(q, 1, true)) then
                local hit = false
    ...
    local function render()
        for _, c in ipairs(list:GetChildren()) do
            if c:IsA("TextButton") or c:IsA("Frame") then
                if c ~= hoverPlay and c ~= hoverHit then c:Destroy() end
            end
        end
        hoverSong = nil
        ...
        for _, song in ipairs(catalog.songs) do
            if matches(song) then
                ...
                corner(row, 4)
                label(row, { Position = UDim2.new(0, 14, 0, 0), Size = UDim2.fromOffset(28, 34) }, tostring(shown), 12, false, C.dim, Enum.TextXAlignment.Left)
                ...
                row.MouseEnter:Connect(function()
                    ...
                    hoverPlay.Position = UDim2.new(row.Position.X.Scale, row.Position.X.Offset + 14, row.Position.Y.Scale, row.Position.Y.Offset + 9)
                    hoverHit.Position = hoverPlay.Position
                end)
    ...
    local function sideItem(name, iconName, glyph, filter)
        ...
        local function setActive(a)
            lbl.TextColor3 = a and C.text or C.sub
            if ic:IsA("ImageLabel") then ic.ImageColor3 = a and C.text or C.sub end
        end
        b.MouseEnter:Connect(function() if filterCat ~= filter then lbl.TextColor3 = C.text end end)
        b.MouseLeave:Connect(function() if filterCat ~= filter then setActive(false) end end)
    ...
    local libItems = {}
    libItems.all = sideItem("All songs", "music", "â™ª", nil)
    libItems.new = sideItem("New", "sparkles", "âœ¦", "__new")
    libItems.featured = sideItem("Featured", "zap", "â˜…", "__featured")
    for _, cat in ipairs(catalog.categories or {}) do
        sideItem(cat, "tag", "â€¢", cat)
    end
    ...
    shuffleBtn.MouseButton1Click:Connect(function()
        if #catalog.songs == 0 then return end
        selectSong(catalog.songs[math.random(1, #catalog.songs)])
    end)
    ...
    local function updateMidiVisibility()
        local inPianoRooms = game.PlaceId == 10888259502
        local show = Settings.data.alwaysshowmidispoofer or inPianoRooms
        midiPill.Visible = show
    end
    ...
    local function toggleSwitch(parent, initial, onChanged)
        ...
        local sw = make("TextButton", { Parent = parent, Size = UDim2.fromOffset(42, 24), BackgroundColor3 = state and C.accent or Color3.fromRGB(90, 90, 90), BorderSizePixel = 0, Text = "", AutoButtonColor = false })
        corner(sw, 12)
    ...
    local function settingRow(title, subtitle, key, iconName, glyph)
        sorder = sorder + 1
        local row = make("Frame", { Parent = settingsList, Size = UDim2.new(1, 0, 0, 46), BackgroundColor3 = C.card, BorderSizePixel = 0, LayoutOrder = sorder })
        corner(row, 8)
        icon(row, iconName, 16, C.sub, UDim2.new(0, 12, 0.5, 0), Vector2.new(0, 0.5), glyph)
        label(row, { Position = UDim2.new(0, 38, 0, 6), Size = UDim2.new(1, -110, 0, 18) }, title, 13, true, C.text)
        label(row, { Position = UDim2.new(0, 38, 0, 24), Size = UDim2.new(1, -110, 0, 16) }, subtitle or "", 10, false, C.sub)
    ...
    settingRow("Autoplay", "Start the song immediately when selected", "autoplay", "play", "â–¶")
    settingRow("Cache songs", "Save songs to the workspace (off = always fetch)", "cachesongs", "box", "â–¢")
    settingRow("Disable notifications", "Hide the in-window toasts", "disablenotifs", "info", "i")
    settingRow("Mute sound effects", "Silence click/play sounds", "mutesfx", "volume-x", "ðŸ”‡")
    settingRow("Precise timing", "Frame-perfect rests (higher CPU)", "secondaryloader", "clock", "â—·")
    settingRow("Disable fake accidents", "Never intentionally miss/shift", "disableaccidents", "shield-check", "âœ“")
    settingRow("Hide featured", "Hide the Featured library item", "disablefeaturedsongs", "sparkles", "âœ¦")
    settingRow("Always show MIDI spoofer", "Show MIDI toggle outside Piano Rooms", "alwaysshowmidispoofer", "zap", "âš¡")
    ...
    notify("0M3G4 P1AN0  â€¢  " .. tostring(#catalog.songs) .. " songs")
end
